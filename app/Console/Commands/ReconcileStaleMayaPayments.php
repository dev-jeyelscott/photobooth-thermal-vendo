...
class ReconcileStaleMayaPayments extends Command
{
    ...
    public function handle(): int
    {
        ...
        if (! is_string($windowMinutes) || ! ctype_digit($windowMinutes) || (int) $windowMinutes < 1) {
        ...
        Payment::query()
            ...
            ->lazyById()
            ->each(function (Payment $payment) use (&$flaggedPayments): void {
                Log::warning('Stale Maya payment requires operator review; success remains webhook-authoritative.', [
        ...
        $this->info("Flagged {$flaggedPayments} stale Maya payment(s) for operator review.");
