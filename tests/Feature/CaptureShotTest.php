...
test('an uploaded capture shot is persisted under the session captures directory', function () {
    ...
    expect($path)
        ->toStartWith(
            'captures/'.$session->session_token.'/',
        );
...
test('an oversized captured shot is downscaled to the configured maximum dimension', function () {
    ...
    config([
        'photobooth.captured_frame_max_dimension_px' => 500,
    ]);
    ...
    $stored = app(ImageManager::class)->decode(
        Storage::disk('public')->get(
            $response->json('path'),
        ),
    );

    expect(max($stored->width(), $stored->height()))
        ->toBe(500)
        ->and($stored->width())
...
test('a captured shot already within bounds is stored unmodified', function () {
    ...
    config([
        'photobooth.captured_frame_max_dimension_px' => 2400,
    ]);
    ...
    $originalContents = file_get_contents(
        $shot->getRealPath(),
    );
